        ...
        while not COORD.should_stop():
            ...
            for t in range(EP_LEN):
                ...
                if t == EP_LEN - 1 or GLOBAL_UPDATE_COUNTER >= self.batch_size:
                    ...
                    if abs(GLOBAL_STEPS-ST_MAX) < 200:
                        COORD.request_stop()
                        #break
                #CSN
...
if __name__ == '__main__':
    ...
    GLOBAL_PPO = PPO(s_dim=4, a_dim=1, actor_lr=0.00001, critic_lr=0.00002, update_steps=10, eps=0.2)

    UPDATE_EVENT, ROLLING_EVENT = threading.Event(), threading.Event()
    UPDATE_EVENT.clear()            # not update now
    ROLLING_EVENT.set()             # start to roll out
    workers = [Worker(wid=i, env=GAME, ppo=GLOBAL_PPO, gamma=0.99, batch_size=320) for i in range(N_WORKER)]
    ...
    save_data('history4_{}'.format(timenow), history)
