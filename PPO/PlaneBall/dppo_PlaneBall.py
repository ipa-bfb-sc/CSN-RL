...
if __name__ == '__main__':
    try:
        ...
        GLOBAL_PPO = PPO(s_dim=7, a_dim=2, actor_lr=0.00001, critic_lr=0.0001, a_update_steps=100, c_update_steps=100, eps=0.2)

        UPDATE_EVENT, ROLLING_EVENT = threading.Event(), threading.Event()
        UPDATE_EVENT.clear()            # not update now
        ROLLING_EVENT.set()             # start to roll out
        workers = [Worker(wid=i, env=GAME, ppo=GLOBAL_PPO, gamma=0.99, batch_size=200) for i in range(N_WORKER)]
