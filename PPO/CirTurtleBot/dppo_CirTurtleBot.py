...
class PPO(object):
    def __init__(self, s_dim, a_dim, actor_lr, critic_lr, update_steps, eps):
        ...
        nn = tf.layer.dense(l1, 24, tf.nn.relu)
        nn = tf.layer.dense(nn, 24, tf.nn.relu)
...
class Worker(object):
    ...
    def work(self):
        ...
        while not COORD.should_stop():
            ...
            print('|W%i' % self.wid, 'steps:{}'.format(GLOBAL_STEPS),'episode:{}'.format(GLOBAL_EP), 'episode steps:{}'.format(t+1),  '|Ep_r: %.2f' % ep_r)
            save_data('history11', history)
...
if __name__ == '__main__':
    ...
    GLOBAL_PPO = PPO(s_dim=4, a_dim=1, actor_lr=0.0001, critic_lr=0.0002, update_steps=10, eps=0.2)

    UPDATE_EVENT, ROLLING_EVENT = threading.Event(), threading.Event()
    UPDATE_EVENT.clear()            # not update now
    ROLLING_EVENT.set()             # start to roll out
    workers = [Worker(wid=i, env=GAME, ppo=GLOBAL_PPO, gamma=0.9, batch_size=96) for i in range(N_WORKER)]
    
    GLOBAL_UPDATE_COUNTER, GLOBAL_EP, GLOBAL_STEPS = 0, 0, 0
    GLOBAL_RUNNING_R = []
    history={'episode':[],'steps':[],'reward':[],'discount_r':[], 'ep_steps':[]}
