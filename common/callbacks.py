...
class TrainEpisodeLogger(Callback):
    ...
    def on_train_begin(self, logs):
        ...
        print('Metrics names: {}'.format(self.metrics_names))
        if self.params['episodes'] == 0:
            print('Training for {} steps ...'.format(self.params['steps']))

        else:
            print('Training for {} episodes ...'.format(self.params['episodes']))

        
    def on_train_end(self, logs):
        """ Print training time at end of training """
        duration = timeit.default_timer() - self.train_start
        print('done, took {:.3f} seconds, total steps: {}'.format(duration, self.step))
    ...
    def on_episode_end(self, episode, logs):
        ...
        with warnings.catch_warnings():
            ...
            for idx, name in enumerate(self.metrics_names):
                if idx > 0:
                    metrics_template += ', '
                try:
                    value = np.nanmean(metrics[:, idx]) # arithmetic average of all steps in one episode, take all rows but keep "idx" column in metrics
                    metrics_template += '{}: {:f}'
                except Warning:
                    value = '--'
                    metrics_template += '{}: {}'
        ...
        template = 'step: {step}: episode: {episode}, duration: {duration:.3f}s, episode steps: {episode_steps}, episode reward: {episode_reward:.3f}, mean action: {action_mean} [{action_min:.3f}, {action_max:.3f}], mean reward: {reward_mean:.3f} [{reward_min:.3f}, {reward_max:.3f}], {metrics}'
        variables = {
            'step': self.step,
            ...
            #'action_last': self.actions[episode][episode_steps - 1],
            #'obs_last': self.observations[episode][episode_steps-1],
