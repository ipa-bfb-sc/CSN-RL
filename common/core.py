...
class Agent(object):
    ...
    def fit(self, env, nb_epsteps=None, nb_steps=None, action_repetition=1, callbacks=None, verbose=1,
        ...
        if (nb_steps is None and nb_epsteps is None):
            raise (ValueError(
                "No duration specified: Please specify one of steps or episodes"
            ))
        elif (nb_steps is not None and nb_epsteps is None):
            step_termination = True
        elif (nb_steps is None and nb_epsteps is not None):
            step_termination = False
        elif (nb_steps is not None and nb_epsteps is not None):
            raise (ValueError(
        ...
        if step_termination == True:
            train_steps = nb_steps
            params = {
                'steps': nb_steps,
                'episodes': 0
            }
        elif step_termination==False:
            train_steps = nb_epsteps
            params = {
                'episodes': nb_epsteps
            }
...
#                        'policy_config': self.get_config()['policy'],
                ...
                # for CartPole termination:
                #if self.training == True and self.num_streaks >= 10:
                # for CirTurtleBot termination:
                #if self.training == True and self.num_streaks >= 5:
                # for PlaneBall termination:
                if self.training == True and self.num_streaks >= 10:
                    self.training = False
                    end_eps = True
                    print('Training end with Episodes:{}'.format(self.episode))
        ...
        train_logs = {'did_abort': did_abort, 'train_end_episode': end_eps}
    ...
    def test(self, env, nb_episodes=1, action_repetition=1, callbacks=None, visualize=True,
        ...
        self._on_test_begin()
        callbacks.on_train_begin()
        for self.episode in range(nb_episodes):
            callbacks.on_episode_begin(self.episode)
            episode_reward = 0.
            episode_step = 0
