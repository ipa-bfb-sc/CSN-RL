...
agent = DDPGAgent(nb_actions=nb_actions, actor=actor, critic=critic, critic_action_input=action_input,
                  memory=memory, nb_steps_warmup_critic=5000, nb_steps_warmup_actor=5000, batch_size=96,
                  random_process=random_process, gamma=.99, target_model_update=1e-3)
agent.compile([Adam(lr=0.00001),Adam(lr=0.001)], metrics=['mae'])

agent.fit(env, nb_steps=300000, visualize=False, callbacks=[callback4], verbose=2)
time.sleep(1800)
