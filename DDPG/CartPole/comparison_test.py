...
### learning rate comparison: 0.001, 0.01,
...
a = np.where(pp1_1['train_end_episode'])[0][0]
print(a)
b=[]

for i in range (700, a):
    b.append(pp1_1['episode_reward'][i])
print(b)

#pyplot.plot(episode_reward1, 'hotpink', label='learn_rate=0.01')
...
pyplot.title('actor learning rate comparison-reward', fontsize=24)

#pyplot.plot(f1_int, 'hotpink', label='actor_lr=0.00001')
pyplot.plot(f_avg2, 'green', label='actor_lr=0.001')
#pyplot.plot(f5_int, 'blue', label='learn_rate=0.001')
pyplot.plot(f_avg1, 'red', label='actor_lr=0.00001')
#pyplot.plot(st_ave3, er_ave3, 'deepskyblue', label='batch_size=96')
pyplot.legend()
pyplot.savefig('save/pics/ddpg_cartpole_lr_reward.png',bbox_inches='tight')
...
pyplot.xticks(x, ('actor_lr=0.00001', 'actor_lr=0.001'))
ax.set_ylabel('Duration')
ax.set_title('Training time with different actor learning rate')
pyplot.savefig('save/pics/ddpg_cartpole_duration_LR.png',bbox_inches='tight')
