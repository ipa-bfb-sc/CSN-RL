...
class DDPGAgent(Agent):
    ...
    def compile(self, optimizer, metrics=[]):
        ...
        print('actor-optimizer:{}'.format(actor_optimizer))
        print('critic-optimizer:{}'.format(critic_optimizer))

        #print(type(critic_optimizer))
        ...
        combined_inputs = [] # state s, action a
        critic_inputs = [] # state s
        for i in self.critic.input:
            if i == self.critic_action_input: # a
                combined_inputs.append([])
            else:
                combined_inputs.append(i) # s
                critic_inputs.append(i)
        combined_inputs[self.critic_action_input_idx] = self.actor(critic_inputs)

        combined_output = self.critic(combined_inputs) # q_predict
        #loss = -tf.reduce_mean(combined_output) # q_predict, from critic online_net, input: s, a
        loss = -K.mean(combined_output)
        #loss = K.mean(K.max(combined_output, axis=-1))

        updates = actor_optimizer.get_updates(params=self.actor.trainable_weights, loss=loss)
        #print('weights:{}'.format(self.actor.trainable_weights))
        #print('updated:{}'.format(updates))
    ...
    def select_action(self, state):
        ...
        if self.training and self.random_process is not None:
            ...
            action += noise
            #print('action+noise:{}'.format(action))
        #print('action-noise:{}'.format(action))
    ...
    def forward(self, observation):
        ...
        #print('test1:action,{}'.format(action))
    ...
    def backward(self, reward, terminal=False):
        ...
        if can_train_either and self.step % self.train_interval == 0:
            ...
            if self.step > self.nb_steps_warmup_actor:
                ...
                #print('test2:action values,{}'.format(action_values))
