...
callback5 = FileLogger(filepath='save/history5_{}'.format(timenow), interval=1)
...
#history1 = dqn1.fit(env, nb_epsteps=3000, visualize=False, callbacks=[callback1], verbose=2)

#dqn2 = DQNAgent(model=model, nb_actions=nb_actions, memory=memory, nb_steps_warmup=2000,
#               target_model_update=1e-2, policy=policy2)
#dqn2.compile(Adam(lr=1e-3), metrics=['mae'])
#history2 = dqn2.fit(env, nb_epsteps=2000, visualize=False, callbacks=[callback2], verbose=2)
...
dqn5 = DQNAgent(model=model, nb_actions=nb_actions, memory=memory, batch_size=96, nb_steps_warmup=2000,
                target_model_update=1e-2, policy=policy2)
dqn5.compile(Adam(lr=1e-3), metrics=['mae'])
history5 = dqn5.fit(env, nb_epsteps=3000, visualize=False, callbacks=[callback5], verbose=2)
