...
callback13 = FileLogger(filepath='save/history13_{}'.format(timenow), interval=1)


dqn8 = DQNAgent(model=model, nb_actions=nb_actions, memory=memory, gamma=0.99, nb_steps_warmup=2000,
                target_model_update=0.5, policy=policy1)
dqn8.compile(Adam(lr=1e-3), metrics=['mae'])
history8 = dqn8.fit(env, nb_epsteps=3000, visualize=False, callbacks=[callback13], verbose=2)
