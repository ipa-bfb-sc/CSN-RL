...
dqn7 = DQNAgent(model=model, nb_actions=nb_actions, memory=memory, gamma=0.5, nb_steps_warmup=2000,
                target_model_update=1e-2, policy=policy2)
dqn7.compile(Adam(lr=1e-3), metrics=['mae'])
history7 = dqn7.fit(env, nb_epsteps=3000, visualize=False, callbacks=[callback7], verbose=2)
