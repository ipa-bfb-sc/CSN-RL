...
dqn4 = DQNAgent(model=model, nb_actions=nb_actions, memory=memory, batch_size=64, nb_steps_warmup=2000,
               target_model_update=1e-2, policy=policy2)
dqn4.compile(Adam(lr=1e-3), metrics=['mae'])
history4 = dqn4.fit(env, nb_epsteps=3000, visualize=False, callbacks=[callback4], verbose=2)
