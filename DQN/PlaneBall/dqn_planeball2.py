...
'''
dqn4 = DQNAgent(model=model, nb_actions=nb_actions, memory=memory, batch_size=640, nb_steps_warmup=20000,
               target_model_update=50000, policy=policy1)
dqn4.compile(Adam(lr=1e-2), metrics=['mae'])
history4 = dqn4.fit(env, nb_epsteps=2500, visualize=False, callbacks=[callback2], verbose=2)
