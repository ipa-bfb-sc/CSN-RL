...
'''
dqn4 = DQNAgent(model=model, nb_actions=nb_actions, memory=memory, nb_steps_warmup=5000,
               target_model_update=1e-2, policy=policy2, enable_double_dqn=False)
dqn4.compile(Adam(lr=1e-3), metrics=['mae'])
history4 = dqn4.fit(env, nb_epsteps=200, visualize=False, callbacks=[callback4], verbose=2)
...
pyplot.plot(history4.history['episode_reward'], 'b')
pyplot.show()