...
dqn3 = DQNAgent(model=model, nb_actions=nb_actions, memory=memory, batch_size=320, nb_steps_warmup=20000,
               target_model_update=1e-2, policy=policy2)
dqn3.compile(Adam(lr=0.5), metrics=['mae'])
history3 = dqn3.fit(env, nb_epsteps=2500, visualize=False, callbacks=[callback10], verbose=2)
