...
from A3C.a3c import ContinuousA3CAgent
...
callback1 = FileLogger(filepath='history1_{}'.format(timenow), interval=1)
...
def run(agent, idx):
    if idx == 0:
        ...
        verbose = 0
    agent.fit(gym.make(ENV_NAME), nb_steps=1000000, visualize=False, verbose=verbose, nb_max_episode_steps=200, callbacks=[callback1])
